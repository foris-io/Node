...
uint8_t message[100]="Hello World1";
char str[40];//String(moistureValue);

/*
int str_len = s.length() + 1;
char str[str_len];
s.toCharArray(str,str_len);
*/
...
void setup()
{
     ...
     while(moistureValue!=0){
      moist[i]=moistureValue%10;
      i++;
  ...
  //delay(2000);
  //sprintf(str,"t%fh%fm%ds%d",DHT.temperature,DHT.humidity,analogRead(A0),analogRead(analogInPin));
...
void loop(void)
{
  ...
  sprintf(str,"t%fh%fm%ds%d",DHT.temperature,DHT.humidity,analogRead(A0),analogRead(analogInPin));
  delay(1000);
  r_size=sprintf((char*)message, str);*/
  delay(1000);
  while (1) {
       int chk = DHT.read11(DHT11_PIN);
      sprintf(str,"t%fh%fm%ds%d",DHT.temperature,DHT.humidity,analogRead(A0),analogRead(analogInPin));
